...
sub mn_os {
	...
	if (!$session->{mn_os}) {
		if ($display_warning) {
			notify($ERRORS{'WARNING'}, 0, "unable to return management node OS object, \$session->{mn_os} is not set");
		}
		return;
	}
	else {
		return $session->{mn_os};
	}
}
...
sub setup_test_rpc_xml {
	...
	if ($session->{rpc_xml_error} =~ /internal server error/i) {
	...
	if ($session->{rpc_xml_error} =~ /internal error while processing/i) {
		# Affiliation not included in username
		# User doesn't exist but affiliation does
		# Affiliation does not exist

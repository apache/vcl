...
sub prepare_vmx {
	...
	if ($virtual_switch_0) {
		if (!grep(/^$virtual_switch_0$/, @network_names)) {
			notify($ERRORS{'WARNING'}, 0, "VM network 0 '$virtual_switch_0' configured in the VM profile does not match any network names on the VM host:\n" . join("\n", sort @network_names));
			return;
		}
		
		if ($vm_eth0_generated) {
			%vmx_parameters = (%vmx_parameters, %{$self->get_generated_ethernet_vmx_definition($interface_index, $virtual_switch_0)});
		}
		else {
			my $vm_eth0_mac = $self->data->get_computer_eth0_mac_address();
			%vmx_parameters = (%vmx_parameters, %{$self->get_static_ethernet_vmx_definition($interface_index, $virtual_switch_0, $vm_eth0_mac)});
		}
		$interface_index++;
	}
	...
	if ($virtual_switch_1) {
		if (!grep(/^$virtual_switch_1$/, @network_names)) {
			notify($ERRORS{'WARNING'}, 0, "VM network 1 '$virtual_switch_1' configured in the VM profile does not match any network names on the VM host:\n" . join("\n", sort @network_names));
			return;
		}
		
		if ($vm_eth1_generated) {
			%vmx_parameters = (%vmx_parameters, %{$self->get_generated_ethernet_vmx_definition($interface_index, $virtual_switch_1)});
		}
		else {
			my $vm_eth1_mac = $self->data->get_computer_eth1_mac_address();
			%vmx_parameters = (%vmx_parameters, %{$self->get_static_ethernet_vmx_definition($interface_index, $virtual_switch_1, $vm_eth1_mac)});
		}
		$interface_index++;
	}
	
	# If vmprofile.virtualswitch2 or vmprofile.virtualswitch3 are defined in the database, add autogenerated interfaces
	if ($virtual_switch_2) {
		if (!grep(/^$virtual_switch_2$/, @network_names)) {
			notify($ERRORS{'WARNING'}, 0, "VM network 2 '$virtual_switch_2' configured in the VM profile does not match any network names on the VM host:\n" . join("\n", sort @network_names));
			return;
		}
		
		%vmx_parameters = (%vmx_parameters, %{$self->get_generated_ethernet_vmx_definition($interface_index, $virtual_switch_2)});
		$interface_index++;
	}
	if ($virtual_switch_3) {
		if (!grep(/^$virtual_switch_3$/, @network_names)) {
			notify($ERRORS{'WARNING'}, 0, "VM network 3 '$virtual_switch_3' configured in the VM profile does not match any network names on the VM host:\n" . join("\n", sort @network_names));
			return;
		}
		
		%vmx_parameters = (%vmx_parameters, %{$self->get_generated_ethernet_vmx_definition($interface_index, $virtual_switch_3)});
		$interface_index++;
	}
...
=head2 get_static_ethernet_vmx_definition
 ...
 Description : Generates a hash containing keys and values which correlate to
               the vmx file parameters necessary to define an ethernet interface
...
sub get_static_ethernet_vmx_definition {
	...
	if (!defined($interface_index)) {
		notify($ERRORS{'WARNING'}, 0, "interface index argument was not supplied");
		return;
	}
	
	my $network_name = shift;
	if (!defined($network_name)) {
		notify($ERRORS{'WARNING'}, 0, "network name argument was not supplied");
		return;
	}
	
	my $mac_address = shift;
	if (!defined($mac_address)) {
		notify($ERRORS{'WARNING'}, 0, "MAC address argument was not supplied");
		return;
	}
	...
	my %vmx_parameters = (
		...
		"ethernet$interface_index.virtualDev" => "$vm_ethernet_adapter_type",
